...
const DOC_TYPE_TO_FILENAME: Record<string, string> = {
  ...
  productionReferences: 'production-references.cineproductionreferences',
};
...
export function serializeAppliedProject(
  ...
  const productionReferencesFilename = 'production-references.cineproductionreferences';
  ...
  const manifest: CineProjectManifest = {
    ...
    documents: {
      ...
      productionReferences: productionReferencesFilename,
    },
  ...
  const allDocuments: Record<string, string> = {
    ...
    [productionReferencesFilename]: JSON.stringify({ references: applied.productionReferences ?? [] }, null, 2),
  };
