...
- [x] Migrate `storyboard-bundle.ts`: `setPrevisSelectionState` imported from `@/data/project-data`. `selectedStoryboardFrameId` imported for reads.
- [x] Migrate `previs-margin.ts`: `renderTimeline`, `storyboardFrames`, `selectStoryboardFrameById` imported from canonical modules.
- [x] Migrate `storyboard-context-menus.ts`: `scheduleFountainRender` imported from `@/script/fountain-bundle`.
- [x] Migrate `workspace-overview-panel.ts`: `getTreeSectionKeyForNode` imported from `@/tree/project-tree-service`.
- [x] Export 6 chip-bundle functions consumed by storyboard-context-menus: `hideChipContextMenu`, `getChipAtScriptCaret`, `showChipContextMenuAt`, `extractChipsFromText`, `navigateStoryboardDestination`, `extractChipsFromTexts`, `renderInspectorChipsSection`. Exported `normalizeEntityName` from utils-bundle. storyboard-context-menus now imports 5 directly (79→16 window.* refs).
- [ ] Replace remaining `window.*` paths: `currentSceneId` (no export), `scheduleScriptEditorProjectSync` (no export), `addItemsToLibrary` (no export), `storyboardContextState` (bridge variable, needs getter/setter), `selectedStoryboardFrameId` writes (needs setter function), `highlightScriptForFrame`/`renderStoryboard` in shot-frame-bridge (circular dep), `extractChipsFromTexts`/`renderInspectorChipsSection` in inspector (circular dep with chip-bundle).
